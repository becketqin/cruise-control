...
public class KafkaAssignerDiskUsageDistributionGoal implements Goal {
  ...
  public boolean optimize(ClusterModel clusterModel, Set<Goal> optimizedGoals, Set<String> excludedTopics)
    ...
    do {
      ...
      for (Broker broker : brokers) {
        if (checkAndOptimize(broker, clusterModel, meanDiskUsage, lowerThreshold, upperThreshold)) {
          improved = true;
        }
      }
      numIterations++;
  ...
  private boolean isOptimized(ClusterModel clusterModel, double upperThreshold, double lowerThreshold) {
    ...
    for (Broker broker : clusterModel.healthyBrokers()) {
      double brokerSize = brokerSize(broker);
      if (brokerSize < lowerThreshold) {
        brokersUnderLowerThreshold.add(broker);
      } else if (brokerSize > upperThreshold) {
        brokersAboveUpperThreshold.add(broker);
      }
    }
    if (!brokersUnderLowerThreshold.isEmpty()) {
      StringJoiner joiner = new StringJoiner(", ");
      brokersUnderLowerThreshold.forEach(b -> joiner.add(String.format("%d:(%.1f)", b.id(), diskUsage(b))));
    ...
    if (!brokersAboveUpperThreshold.isEmpty()) {
      StringJoiner joiner = new StringJoiner(", ");
      brokersAboveUpperThreshold.forEach(b -> joiner.add(String.format("%d:(%.1f)", b.id(), diskUsage(b))));
  ...
  private boolean checkAndOptimize(Broker broker,
    ...
    if (brokerDiskUsage > upperThreshold) {
      ...
      for (Broker toSwapWith : brokersAscend) {
        ...
        if (swapReplicas(broker, toSwapWith, meanDiskUsage, clusterModel)) {
          improved = true;
          break;
        }
      }
   ...
   * @throws AnalysisInputException
   */
  boolean swapReplicas(Broker toSwap, Broker toSwapWith, double meanDiskUsage, ClusterModel clusterModel) 
      throws AnalysisInputException {
    ...
    List<ReplicaWrapper> sortedReplicasToSwap = sortReplicasAscend(toSwap.replicas());
    List<ReplicaWrapper> sortedLeaderReplicasToSwapWith = sortReplicasAscend(toSwapWith.leaderReplicas());
    List<ReplicaWrapper> sortedFollowerReplicasToSwapWith = sortReplicasAscend(followerReplicas(toSwapWith));
    ...
    for (int i = startPos; i >= 0 && i < sortedReplicasToSwap.size(); i += delta) {
      ...
      List<ReplicaWrapper> sortedReplicasToSwapWith = 
          replicaToSwap.isLeader() ? sortedLeaderReplicasToSwapWith : sortedFollowerReplicasToSwapWith;
  ...
  Replica findReplicaToSwapWith(Replica replica,
    ...
    while (pos >= minPos && pos <= maxPos) {
      Replica toSwap = sortedReplicasToSearch.get(pos).replica();
   ...
   */
  private List<ReplicaWrapper> sortReplicasAscend(Collection<Replica> replicas) {
    List<ReplicaWrapper> sortedReplicas = new ArrayList<>();
    replicas.forEach(r -> sortedReplicas.add(new ReplicaWrapper(r, replicaSize(r))));
