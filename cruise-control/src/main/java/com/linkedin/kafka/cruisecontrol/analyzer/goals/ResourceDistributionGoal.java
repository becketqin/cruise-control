  ...
  protected double brokerMetricValueAfterAction(Broker broker, BalancingAction action, ClusterModel clusterModel) {
    ...
    switch (action.actionType()) {
      ...
      case REPLICA_MOVEMENT:
        if (broker.id() == srcReplica.broker().id()) {
          double sourceBrokerLoad = srcReplica.broker().load().expectedUtilizationFor(resource());
          return sourceBrokerLoad - srcReplica.load().expectedUtilizationFor(resource());
        } else if (broker.id() == destBroker.id()) {
          double destBrokerLoad = destBroker.load().expectedUtilizationFor(resource());
          return destBrokerLoad + srcReplica.load().expectedUtilizationFor(resource());
        } else {
          return broker.load().expectedUtilizationFor(resource());
        }
      case REPLICA_SWAP:
        ...
        if (broker.id() == srcReplica.broker().id()) {
          return srcBrokerLoad - destReplicaLoad - srcReplicaLoad;
        } else if (broker.id() == destBroker.id()) {
          return destBrokerLoad + srcReplicaLoad - destReplicaLoad;
        } else {
          return broker.load().expectedUtilizationFor(resource());
        }
      default:
