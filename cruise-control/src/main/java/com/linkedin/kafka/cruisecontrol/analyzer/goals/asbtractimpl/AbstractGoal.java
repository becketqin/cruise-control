...
public abstract class AbstractGoal implements Goal {
  ...
  protected Broker maybeApplyBalancingAction(ClusterModel clusterModel,
    ...
    for (Broker broker : candidateBrokers) {
      ...
      LOG.trace("Trying to apply legit and self-satisfied action {}, actionAcceptance = {}", balancingAction, acceptance);
  ...
  protected void applyBalancingAction(BalancingAction balancingAction,
                                      BrokerAndSortedReplicas srcBrokerReplica,
                                      BrokerAndSortedReplicas destBrokerReplica,
                                      ClusterModel clusterModel) {
    LOG.debug("Applying action {} to the cluster", balancingAction);
    switch (balancingAction.actionType()) {
      case LEADERSHIP_MOVEMENT:
        moveLeader(balancingAction, srcBrokerReplica, destBrokerReplica, clusterModel);
        break;
      case REPLICA_MOVEMENT:
        moveReplica(balancingAction, srcBrokerReplica, destBrokerReplica, clusterModel);
        break;
      case REPLICA_SWAP:
        swapReplicas(balancingAction, srcBrokerReplica, destBrokerReplica, clusterModel);
        break;
      default:
  ...
  private void moveLeader(BalancingAction balancingAction,
                          BrokerAndSortedReplicas srcBrokerReplicas,
                          BrokerAndSortedReplicas destBrokerReplicas,
                          ClusterModel clusterModel) {
    ...
    Replica srcReplica = srcBrokerReplicas.broker().replica(tp);
    Replica destReplica = destBrokerReplicas.broker().replica(tp);
    srcBrokerReplicas.add(srcBrokerReplicas.remove(srcReplica));
    destBrokerReplicas.add(destBrokerReplicas.remove(destReplica));
  }

  private void moveReplica(BalancingAction balancingAction,
                           BrokerAndSortedReplicas srcBrokerReplicas,
                           BrokerAndSortedReplicas destBrokerReplicas,
                           ClusterModel clusterModel) {
    moveReplica(balancingAction.topicPartition(), srcBrokerReplicas, destBrokerReplicas, clusterModel);
  }

  private void swapReplicas(BalancingAction balancingAction,
                            BrokerAndSortedReplicas srcBrokerReplicas,
                            BrokerAndSortedReplicas destBrokerReplicas,
                            ClusterModel clusterModel) {
    moveReplica(balancingAction.topicPartition(), srcBrokerReplicas, destBrokerReplicas, clusterModel);
    moveReplica(balancingAction.destinationTopicPartition(), destBrokerReplicas, srcBrokerReplicas, clusterModel);
  }

  private void moveReplica(TopicPartition tp,
                           BrokerAndSortedReplicas srcBrokerReplicas,
                           BrokerAndSortedReplicas destBrokerReplicas,
                           ClusterModel clusterModel) {
    Replica srcReplica = srcBrokerReplicas.broker().replica(tp);
    srcBrokerReplicas.remove(srcReplica);
    clusterModel.relocateReplica(tp, srcBrokerReplicas.broker().id(), destBrokerReplicas.broker().id());
    Replica destReplica = destBrokerReplicas.broker().replica(tp);
    destBrokerReplicas.add(destReplica);
  }
  ...
  protected ActionAcceptance isBalancingActionAcceptable(ClusterModel clusterModel,
    ...
    if (!isBalancingActionLegit(clusterModel, balancingAction)) {
      ...
      return AnalyzerUtils.isProposalAcceptableForOptimizedGoals(optimizedGoals, balancingAction, clusterModel);
    }
  }
   ...
   */
  protected boolean isBalancingActionAppropriate(ClusterModel clusterModel, BalancingAction balancingAction) {
    if (!clusterModel.selfHealingEligibleReplicas().isEmpty()) {
      ...
      switch (balancingAction.actionType()) {
        ...
        case REPLICA_SWAP:
          ...
          return (srcBroker.isNew() && destBroker.isNew()) // both end are new
