...
public class LeaderMovementReplicaFinder extends DistributionGoalReplicaFinder {
  ...
  LeaderMovementReplicaFinder(Broker brokerToOptimize,
    ...
    if (targetValueForBrokerToOptimize > currentValueForBrokerToOptimize) {
      // broker to optimize needs more load. Search in the broker to balance with for leaders.
      _brokerToSearch = brokerToBalanceWith;
    } else {
      // Broker to optimize needs less load. Search in the broker to optimize for leaders.
