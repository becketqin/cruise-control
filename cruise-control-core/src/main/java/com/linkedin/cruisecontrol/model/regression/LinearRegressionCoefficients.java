...
public class LinearRegressionCoefficients {
   ...
   */
  LinearRegressionCoefficients(CausalRelation causalRelation, int maxExponent) {
  ...
  private StringBuilder appendWithExponents(StringBuilder sb, Map<MetricInfo, Double> coefficients, int exponent) {
    ...
    for (Map.Entry<MetricInfo, Double> entry : coefficients.entrySet()) {
      ...
      sb.append(String.format("%s%.4f * %" + metricPrintLength + "s^%d%s", 
                              entry.getValue(), entry.getKey(), exponent, 
                              exponent == _coefficients.size() ? "" : " + "));
    }
    return sb;
  }

  /**
   * Make the order string grammatically correct.
   */
