...
class DataForLinearRegression {
  ...
  private final AtomicInteger _metricsCount;
  ...
  DataForLinearRegression(CausalRelation causalRelation,
    ...
    _metricsCount = new AtomicInteger(0);
  }
  ...
  void addValues(double[] metricValues) {
    int index = _metricsCount.getAndIncrement();
    ...
    for (int i = 1; i <= _maxExponent; i++) {
      for (MetricInfo metric : _causalRelation.causalMetrics()) {
        int causalMetricIndex = _causalRelation.index(metric) + (i - 1) * _causalRelation.causalMetrics().size();
        double metricValue = metricValues[causalMetricIndex];
        _causalMetricsValues[index][causalMetricIndex] = Math.pow(metricValue, i);
      }
    }
  }
